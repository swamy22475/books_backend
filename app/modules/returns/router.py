...
async def create_return(
    ...
    return crud.create_return(db=db, tenant_id=tenant_id, return_data=return_data)
...
async def read_returns(
    ...
    return crud.get_returns(db, tenant_id=tenant_id, skip=skip, limit=limit)
...
async def read_return(
    ...
    db_return = crud.get_return(db, tenant_id=tenant_id, return_id=return_id)
    if not db_return:
...
async def update_return(
    return_id: int, 
    return_data: schemas.ReturnUpdate, 
    db: Session = Depends(get_db),
    tenant_id: str = Depends(get_tenant_id)
):
    db_return = crud.update_return(db=db, tenant_id=tenant_id, return_id=return_id, return_data=return_data)
    if not db_return:
        raise HTTPException(status_code=404, detail="Return entry not found")
    return db_return
...
async def delete_return(
    ...
    success = crud.delete_return(db=db, tenant_id=tenant_id, return_id=return_id)
    if not success:
        raise HTTPException(status_code=404, detail="Return entry not found")
